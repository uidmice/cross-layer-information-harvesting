...
class Node:
    ...
    def create_unique_packet(self, payload, adr=False, adrAckReq=False):
        packet = UplinkPacket(self, self._unique_packet_id, payload, adr, adrAckReq)
        self._unique_packet_id += 1
        return packet

    def send(self, packet, status=None):
        ...
        if packet.received:
            ...
            if status:
                status['successful_transmission'] += 1
        else:
            self.last_payload_change = 0
            self.last_packet_success = False
...
class UplinkPacket():
    ...
    def __init__(self, node: Node, id, payload=0, adr=True, adrAckReq=False):
        self.id = UplinkPacket.counter
        UplinkPacket.counter += 1
        self.node = node
        self.para = node.para
    ...
    def airtime(self, toa=None):  # in ms
        if toa:
            self.time_on_air = toa
            return toa
